...
<body>
			...
			<style>  .table input{width:20px;height:13px}</style>
			<script>
			$(function(){
				var account="${account}";
				...
				var locationId;
				var bounced=true;
				function showtarding(){
					
					$.post("${pageContext.request.contextPath}/customer/GetLocationServlet",{},function(data){
					   data=eval(data);
						var location=data[0];
						console.log(location);
						var a="";
						if(location.defaultLocation==0){
		        			a="不是";
		        		}else{
		        			a="是";
		        		}
			    		...
			    		for(var i=0;i<location.length;i++){
			    			table+="<tr><td>"+"收货地址：" +location[i].consignee
			    			+"—"+location[i].phone
			    			+"—"+location[i].provinces
			    			+"/"+location[i].city
			    			+"/"+location[i].county
			    			+"/"+location[i].specificLocation
			    			+"<input id='defaultLocation' type='radio' name='default'>"+"是否设为默认地址"
			    			+"<tr><td>"+"<a id='Update' href='"+location[i].locationId+"'>修改收货地址</a>"
			    			+"</td></tr>";
			    		}
				...
				$(document).on("click","#Update",function(event){
			   		...
			   		bounced=false;
			   	 $("#renyuan").addClass("show");
			   	locationId=$(this).attr("href");
			   	var provinces=$("#provinces").val();
			   	var city=$("#city").val();
			   	var county=$("#county").val();
			   	var specificLocation=$("#specificLocation").val();
			   	var consignee=$("#provinces").val();
			   	var phone=$("#provinces").val();
			   	var param={
			   	...
			   	console.log(locationId);
		   		 $.post("${pageContext.request.contextPath}/customer/GetUpdateLocation",param,function(data){
		   			 data=eval(data);
		   			 var updateLocation=data[0];
		   			 console.log(updateLocation);
		   			$("#provinces  option[value='"+updateLocation.provinces+"']").prop("selected",true);
		   			$("#city  option[value='"+updateLocation.city+"']").prop("selected",true);
		   			$("#county  option[value='"+updateLocation.county+"']").prop("selected",true);
		   			$("#specificLocation").val(updateLocation.specificLocation);
		   			$("#consignee").val(updateLocation.consignee);
		   			$("#phone").val(updateLocation.phone);
		   		 }); 
		   		return false;
				});
				$("#add").click(function(){
					bounced=true;
					$("#renyuan").addClass("show");
					$("#provinces  option[value='"+updateLocation.provinces+"']").prop("selected",false);
		   			$("#city  option[value='"+updateLocation.city+"']").prop("selected",false);
		   			$("#county  option[value='"+updateLocation.county+"']").prop("selected",false);
				...
				$("#add_btn").click(function(){
					...
					var specificLocation=$("#specificLocation").val();
					if(null==specificLocation||""==specificLocation){
		    		    alert("详细地址不能为空");
		   			    return false; 
		    		  } 
					var consignee=$("#consignee").val();
					if(null==consignee||""==consignee){
		    		    alert("收货人姓名不能为空");
		   			    return false; 
		    		  } 
					var phone=$("#phone").val();
					...
					var defaultLocation=0;
				 	if ($("#defaultLocation").prop("checked")){
				 		defaultLocation=1;
	        		}
				 	locationId=locationId;
					var param={ 
					...
					if(bounced==false||bounced=="false"){
						$.post("${pageContext.request.contextPath}/customer/updateLocationServlet",param,function(data){
					...
					$.post("${pageContext.request.contextPath}/customer/DoInsLocationServlet",param,function(data){
						if(data==true||data=="true"){
			    			alert("添加成功");
			   		    }else{
...
<!-- banner -->
			...
			<select id="provinces"><option>省份</option></select>
			<select id="city"><option>地级市</option></select>
			<select id="county"><option>市、县级市、县</option></select>
			<br>
			<br>
			详细地址：<input id="specificLocation" type="text" style=" width:235px">
			<br>
			<br>
			收&nbsp;货&nbsp;人&nbsp;:&nbsp;<input id="consignee" type="text" style=" width:235px">
			<br>
			<br>
			电&nbsp;&nbsp;&nbsp;&nbsp;话&nbsp;&nbsp;:&nbsp;<input id="phone" type="number" style=" width:235px">
			<br>
				<input id="defaultLocation" type="CheckBox">是否设置为默认地址
			</form>
                            
                        </div>
                        <div class="modal-footer">
                            <button type="button" style="background:white !important" id="cancel" class="btn btn-secondary" data-dismiss="modal">取消</button>
...
<!-- mail -->
		<div class="pay" style="background:white">
	<!--主内容开始编辑-->
	<div><h1 style="text-align: center">收货地址管理</h1>
	<br>
	<button id="add" type="button" style="margin-left:10px" > 添加收货地址</button>	</div>
