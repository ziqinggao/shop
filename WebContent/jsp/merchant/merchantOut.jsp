  ...
  <head>
    ...
    <script src="${pageContext.request.contextPath }/js/jquery-3.3.1.min.js"></script>
     <script type="text/javascript">
        	$(function(){
        		$("#wd").click(function(){
        			var merchantBalance = $("#wdBalance").val();
        			if(merchantBalance != null){
        			var param = {
        					merchantBalance:merchantBalance
        			}
        			$.post("${pageContext.request.contextPath}/merchant/MerchantBalanceOutServlet",param,function(data){
        				if(data == true||data == "true"){
        					alert("提现成功");
        					window.location.href = "${pageContext.request.contextPath}/jsp/merchant/merchantOut.jsp";
        				}else{
        					alert("提现失败");
        				}
        			});
...
<div class="odform">
	<form action="#">
		...
		<div class="input-group">
			...
			<input type="text" name="merchantBalance" id="wdBalance" placeholder="请输入您的提现金额">
		</div>
		<div class="input-group">
			<label for="khname">预约时间</label>
			<input type="date" class="cal" id="khname" placeholder="请选择提现时间">
		</div>
		<div class="input-group">
			<label for="khname">附加备注</label>
			<input type="text" id="khname" placeholder="备注">
		</div>
		<button type="button" id="wd">提现</button>
	</form>
</div>
          <!-- Page Header-->
