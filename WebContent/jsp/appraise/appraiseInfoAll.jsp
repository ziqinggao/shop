  ...
  $(function(){
		  ...
		  function foundOption(){
				//获取搜索输入框的值
	  			...
	  			var vaild=0;
	  			if($("#Ccheck").prop("checked")==true){
	  				vaild=1;
	  			}
	  			$.post("${pageContext.request.contextPath}/appraise/getMaxPagesServlet",{commodityName:commodityName,vaild:vaild},function(data){
	  				 var option="";
	  				  maxpages=data;
		    ...
		    function showCommodity(){
			  		...
			  		var pages=$(".pages").val();//获取当前页数
		  			
		  			var commodityName=$("#Ktext").val().trim();//获取搜索输入框的值
		  			...
		  			/**<th>商品名称</th>
									...
									<th>评价日期</th>
									<th>评价人</th>*/
		  			$.post("${pageContext.request.contextPath}/appraise/getEvaluateVByCommodityNameServlet",param,function(data){
		  				data=eval(data);
		  				var evaluateV=data[0];
		  				table="";
		  				for(var i=0;i<evaluateV.length;i++){
		  					table+="<tr><td>"+evaluateV[i].commodityName
		  							+"</td><td>"+evaluateV[i].subsort
		  							+"</td><td>"+evaluateV[i].commodityScore
		  							+"</td><td>"+evaluateV[i].logisticScore
		  							+"</td><td>"+evaluateV[i].serviceScore
		  							+"</td><td>"+evaluateV[i].evaluate
		  							+"</td><td>"+evaluateV[i].evlauateDate
		  							+"</td><td>"+evaluateV[i].accountName+"**</td>"
							...
							<div class="person_search">
								<div class="search_input">
									<div class="input-group mb-3">
										...
										<input id='Ccheck' type='checkbox' checked='checked' style="width:20px ! important ;margin-left:15px"><span style="color:black;margin-left:0px">过滤简短评价</span>
									</div>
								</div>
								<div class="search_input">
				...
				<div class="heardstyle">
					...
					<div>
						<svg class="waves" xmlns="http://www.w3.org/2000/svg"
							xmlns:xlink="" viewBox="0 24 150 28"
